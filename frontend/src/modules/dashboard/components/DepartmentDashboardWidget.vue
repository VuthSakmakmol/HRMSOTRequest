<!-- frontend/src/modules/dashboard/components/DepartmentDashboardWidget.vue -->
<script setup>
// frontend/src/modules/dashboard/components/DepartmentDashboardWidget.vue
...
import { getDepartmentDashboardSummary } from '../dashboard.api'
...
const hasDepartmentData = computed(() => Number(summary.value.total || 0) > 0)
...
async function fetchSummary() {
  ...
  try {
    const res = await getDepartmentDashboardSummary()
    ...
    <div class="p-3">
      ...
      <div class="grid gap-3 md:grid-cols-3">
        ...
        <div class="dashboard-stat-card dashboard-stat-card-active">
          ...
          <p v-else class="dashboard-stat-value text-emerald-500">
            {{ summary.active }}
          </p>
        </div>
      ...
      <div class="mt-3 grid gap-3 xl:grid-cols-5">
        <div class="dashboard-chart-card xl:col-span-2">
          ...
          <div class="dashboard-chart-box">
            ...
            <Chart
              v-else-if="chartsReady && hasDepartmentData"
              :key="`status-${chartRenderKey}`"
              type="doughnut"
        ...
        <div class="dashboard-chart-card xl:col-span-3">
          ...
          <div class="dashboard-chart-box">
            ...
            <div v-else class="dashboard-chart-empty">
              No new departments in the last 6 months.
            </div>
          </div>
        </div>
      </div>
...
.dashboard-stat-card-active {
  background:
    linear-gradient(
      135deg,
      color-mix(in srgb, #10b981 10%, var(--ot-surface)),
      var(--ot-surface)
    );
}
